...
#include "../UI/Impl/MapCellGraphicsRectItem.h"
#include "../UI/Impl/MapGraphicsScene.h"
#include "../UI/Impl/MapGraphicsView.h"
...
namespace Camps1te::Editor {
    ...
    UI::Impl::MapGraphicsScene* scene;
    ...
    std::unordered_map<int, std::unordered_map<int, UI::Impl::MapCellGraphicsRectItem*>> sceneCells;
    ...
    class App {
        ...
        void RenderTileColor(UI::Impl::MapCellGraphicsRectItem* rect, const nlohmann::json& color) {
            auto red   = color[0];
            auto green = color[1];
            auto blue  = color[2];
            auto alpha = color[3];
        ...
        void RenderTileImage(UI::Impl::MapCellGraphicsRectItem* rect, const nlohmann::json& image) {
        ...
        void RenderTileLayer(UI::Impl::MapCellGraphicsRectItem* rect, const nlohmann::json& layer) {
        ...
        void RenderTile(UI::Impl::MapCellGraphicsRectItem* rect, const nlohmann::json& tile) {
        ...
        void UpdateMapViewTexture(UI::Impl::MapCellGraphicsRectItem* mapCellRect, int x, int y) {
        ...
        void UpdateMapViewCellColor(UI::Impl::MapCellGraphicsRectItem* mapCellRect, int x, int y) {
        ...
        void UpdateCellProperties(UI::Impl::MapCellGraphicsRectItem* mapCellRect, int x, int y) {
        ...
        void RemoveIndexFromLayersOfMapCell(
            UI::Impl::MapCellGraphicsRectItem* mapCellRect, int x, int y, int index
        ) {
            auto mapCellInfo = GetMapCellInfo(x, y);
            if (mapCellInfo.is_null()) return;
            if (!mapCellInfo.contains("layers")) return;
        ...
        void OnMapViewCellClick(UI::Impl::MapCellGraphicsRectItem* mapCellRect, int x, int y) {
            UpdateCellProperties(mapCellRect, x, y);
            auto selectedTextureName = GetCurrentlySelectedTextureNameOrEmpty();
            auto selectedColorName   = GetCurrentlySelectedColorNameOrEmpty();
    ...
    public:
        int Run(int argc, char* argv[]) {
            ...
            auto cellSize = 128;
            auto padding  = 0;
            scene         = new UI::Impl::MapGraphicsScene();
            for (int i = 0; i < columns; i++) {
                for (int j = 0; j < rows; j++) {
                    auto* rect = new UI::Impl::MapCellGraphicsRectItem(
            ...
            auto* mapView = new UI::Impl::MapGraphicsView();
